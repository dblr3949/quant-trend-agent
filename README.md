...
前端 topbar 会展示当前应用版本，例如 `v0.5.3`。后端接口也会返回版本：
...
- `账户净值`：必填，用于计算仓位比例和总杠杆。
- `现金`：可留空；留空时用 `账户净值 - 当前持仓市值` 粗估，可为负数。
- `融资`：可留空；留空时用 `max(0, 当前持仓市值 - 账户净值)` 粗估。
- `维持保证金`：可留空；有券商准确值就填，留空时按当前持仓市值粗估并标注来源。
- `目标杠杆`：可留空；填写后也只是用户偏好，不是硬命令；更保守会采纳，更激进会被市场和风控截断。
...
用户填写的目标杠杆会作为参考；不填则用市场状态自动目标：

- 如果用户目标更保守，通常会采纳。
- 如果用户目标更激进，会被市场状态、最大杠杆和保证金约束截断。

现金、融资和维持保证金都可以不填。系统会在拿到实时行情后用持仓市值反推：

```text
估算现金 = 账户净值 - 当前持仓市值
估算融资 = max(0, 当前持仓市值 - 账户净值)
估算维持保证金 ≈ 当前持仓市值 × 估算维持保证金率
```

估算值会在结果里标注来源，只作为预算和风险检查参考；券商给出的准确维持保证金优先级更高。
