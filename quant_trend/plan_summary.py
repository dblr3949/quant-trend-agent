...
def _call_openai_summary(compact: dict, *, effort_override: str | None = None, format_retry: bool = False, model: str | None = None) -> dict | None:
    ...
    system = (
        ...
        "如果 options_analysis 启用，必须按标的简短解释期权链结构；优先使用每个标的的 symbol_summary、risk/anomaly、risk_alerts 和 signals，"
        "再补充 OI PCR、成交PCR、近月ATM IV、25D偏斜、最大痛点、OI墙、简化GEX和流动性。"
        "期权部分要翻译成“风险提示/异动提示”，不要只罗列指标。"
