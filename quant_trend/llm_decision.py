...
def _compact_technical(item: dict | None) -> dict:
    ...
    return {
        ...
        "order_flow": item.get("order_flow"),
        "supports": [_compact_level(level) for level in (item.get("supports") or [])[:8]],
        "resistances": [_compact_level(level) for level in (item.get("resistances") or [])[:8]],
...
def _compact_prompt_overlay(overlay: dict | None) -> dict:
    ...
    for symbol, raw in (overlay.get("symbols") or {}).items():
        ...
        symbols[str(symbol).upper()] = {
            ...
            "soft_no_reduce": raw.get("soft_no_reduce"),
            "thesis_status": raw.get("thesis_status"),
            "trade_plan": raw.get("trade_plan"),
            "target_net_exposure": raw.get("target_net_exposure"),
            "buy_condition": raw.get("buy_condition"),
            "sell_condition": raw.get("sell_condition"),
            "risk_trigger": raw.get("risk_trigger"),
    ...
    return {
        ...
        "prompt_parser": overlay.get("prompt_parser"),
...
def _call_openai_decisions(compact: dict, model: str | None = None) -> dict | None:
    model = model or os.getenv("OPENAI_DECISION_MODEL") or os.getenv("OPENAI_MODEL", "deepseek-v4-pro")
    ...
    system = (
        ...
        "必须同时考虑个股量价、筹码/成交占比、支撑压力力度、自动锚定VWAP、多周期风险调整动量、区间波动率、订单流/VPIN近似、SPY/SMH/SOXX 趋势、^VIX 历史分位/Z-score/5日变化、杠杆和保证金。"
