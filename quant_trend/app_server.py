    ...
    def _build_scorecard(self, plan: dict, research: dict) -> dict:
        ...
        return {
            "price_volume": {
                ...
                "verdict": "主研究项",
                "detail": "多窗口支撑压力、Volume Profile/POC/VAH/VAL/HVN/LVN、自动锚定VWAP、筹码占比、量比、订单流/VPIN近似和当日趋势。",
        ...
        return [
            {
                ...
                "detail": "支撑/压力、Volume Profile、筹码占比、自动锚定VWAP、高量成交区、量比、多周期风险调整动量、区间波动率、订单流/VPIN近似和日内趋势会进入 LLM 摘要与前端点位图。",
            },
            {
                "name": "仓位/杠杆/维持保证金",
                "weight": "硬约束",
    ...
    def run_plan(self, payload: dict, kind: str = "manual", user_id: int | None = None) -> dict:
        ...
        try:
            with self.lock:
                ...
                self._progress_step(
                    "量价结构完成",
                    f"生成 {price_volume_count} 个标的的支撑/压力、Volume Profile、自动锚定VWAP、多周期动量、区间波动率、订单流/VPIN近似和点位解释。",
                ...
                self._progress_step("LLM 点位复核", "把候选支撑/压力、筹码占比、自动锚定VWAP、多周期动量、区间波动率、订单流/VPIN近似、指数趋势、VIX波动率风险和风控输入 LLM；主点位只允许从候选价中选择。", {"状态": "开始"})
