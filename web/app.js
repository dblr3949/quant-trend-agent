...
const marketProxySymbols = new Set(["SPY", "SMH", "SOXX", "^VIX"]);
...
function renderResearchFlow(process, plan = null) {
  ...
  const hasEstimatedAccount = !!plan?.portfolio?.account_fields_estimated;
  const llmStep = findStep(steps, /LLM 点位复核完成|LLM 点位复核跳过|LLM 点位复核/);
  const cards = [
    {
      title: "1. 持仓与账户输入",
      status: processStatusBadge("手动表格", "ok"),
      body: "持仓、股数、成本和账户净值来自网页表格；现金、融资、维持保证金可留空，系统会按当前持仓市值粗估并标注来源。个股偏好统一写在“本次调仓想法”里；行情源不读取账户。",
      metrics: [
        processMetric("持仓来源", sourceStatus(sources, "portfolio", "BBAE 手动", "未记录")),
        processMetric("账户字段", hasEstimatedAccount ? "部分估算" : "手动/默认", "现金=净值-持仓市值；融资=max(0, 持仓市值-净值)；维持保证金≈持仓市值×估算维持保证金率。"),
    ...
    },
    {
      ...
      method: "口径：5/10/20/50/63/90/126/180/252日结构、Volume Profile 的 POC/VAH/VAL/HVN/LVN、自动锚定VWAP、筹码占比、成交占比、共振、近252日触碰次数、20/60/120日风险调整动量、ATR/Parkinson/Garman-Klass/Yang-Zhang 区间波动率、订单流/VPIN近似、20日量比、VWAP/近30分钟。",
    },
    {
      title: "4. 仓位与杠杆预算",
      status: processStatusBadge("硬约束", "danger"),
      body: "目标杠杆先由市况分决定：总分 >=4 为 risk_on，默认 1.65x；<=-2 为 risk_off，默认 0.85x；中间为 neutral，默认 1.25x，且不超过最大杠杆。你填写的目标杠杆是弱约束：更保守直接采用，更激进会按市况加成上限截断。",
      metrics: [
        scoreMetric(scorecard, "risk", "杠杆/保证金"),
        processMetric("目标总仓", "市况分映射", "risk_on=1.65x，neutral=1.25x，risk_off=0.85x；用户目标只作弱约束。"),
        processMetric("数量来源", "预算倒推", "目标杠杆 × 净值 -> 目标总仓；目标市值 - 当前市值 -> 可买/需卖金额；再按限价换算股数。"),
      ],
      method: "个股目标权重来自基准权重×趋势/量价/当日/信心/桶/约束乘数，然后归一到目标总杠杆；强制项：不超过最大杠杆，维持保证金安全垫不足时限制新增买单，卖单不超过持仓股数。",
    },
    {
      title: "5. 你的想法与弱覆盖因子",
    ...
    },
    {
      ...
      body: "后端先生成候选支撑/压力价、2-3档参考价梯和预算，LLM 读取这些结构、指数状态、仓位变化和你的 prompt 做复核；原则上只在候选价里选或解释，不凭空编新价格；最终再生成中文重点总结。",
      metrics: [
...
function renderResearchProcess(process, plan = null) {
  ...
  target.innerHTML = `
    ${renderResearchFlow(process, plan)}
...
function levelMetricText(level) {
  ...
  if (level.touch_count !== undefined && level.touch_count !== null) chips.push(`触碰252日 ${level.touch_count}`);
...
function levelMetricBadges(level) {
  ...
  if (level.touch_count !== undefined && level.touch_count !== null) badges.push(metricBadge("触碰252日", level.touch_count, levelHelpText(level, "touch")));
...
function renderVolumeBars(item) {
  ...
  return `
    <div class="volume-bars" title="最近30根日线成交量，绿色为收涨，红色为收跌。">
      ${rows
        .map((row) => {
...
function renderMiniTechChart(item) {
  ...
  const pointCircles = rows
    ...
    .join("");
  return `
    <div class="mini-tech-chart" title="最近30根日线 + 当前价：上方为收盘价走势，下方为成交量柱。">
      <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeHtml(item.symbol)} 最近30根日线和当前价">
          ...
          return `
            <article class="tech-card">
              ...
              <div class="level-columns">
                <div>
                  <h4>支撑</h4>
                  <ul>${renderLevelList(item.supports, "暂无近端支撑", 8)}</ul>
                </div>
                <div>
                  <h4>压力</h4>
                  <ul>${renderLevelList(item.resistances, "暂无近端压力", 8)}</ul>
                </div>
              </div>
...
function renderLargePriceVolumeChart(item) {
  ...
  const candles = rows
    ...
    .join("");
  const rawLineItems = [
    ...sortedLevels(item.supports || []).slice(0, 6).map((level) => ({ ...level, type: "support", label: "支撑" })),
    ...sortedLevels(item.resistances || []).slice(0, 6).map((level) => ({ ...level, type: "resistance", label: "压力" })),
  ]
  ...
  return `
    <div class="large-chart">
      <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeHtml(item.symbol)} 近180根日线价量结构大图">
...
function renderRun(plan) {
  ...
  $("metricRegime").title = "市况分由 SPY/SMH/SOXX 均线与当日趋势、^VIX 波动率状态、弱覆盖事件合成；>=4 risk_on，<=-2 risk_off，其余 neutral。";
  $("metricGross").title = "当前杠杆=当前持仓总市值/账户净值；目标杠杆由市况自动生成，用户输入只作为弱约束。";
...
function drawExposure(plan) {
  ...
  const targetReason = (plan.regime?.reasons || []).find((reason) => String(reason).startsWith("user_target_gross_"));
  const targetTitle = targetReason
    ? `目标杠杆 ${targetGross.toFixed(2)}x；市况自动目标叠加你的弱约束：${translateReason(targetReason)}`
    : `目标杠杆 ${targetGross.toFixed(2)}x；由市况分映射：risk_on 1.65x、neutral 1.25x、risk_off 0.85x。`;
  ...
  target.innerHTML = `
    ...
    <div class="gauge-track" aria-label="当前杠杆 ${gross.toFixed(2)}x，目标 ${targetGross.toFixed(2)}x，上限 ${max.toFixed(2)}x">
      ...
      <span class="gauge-marker target" style="left:${targetPct}%" title="${escapeHtml(targetTitle)}"></span>
